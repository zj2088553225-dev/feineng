<template>
  <a-card style="padding: 10px" title="独立站订单审核明细页">
...
const fetchAllCustomizeOrderData = async (customize_order_type = activeKey.value) => {
  ...
  const pageSize = 200;
  let page = 1;
  const maxPages = 100; // 最多 100 页，即 20000 条
  let totalCount = null;
  ...
  while (page <= maxPages) {
    try {
      ...
      const res = await request.get('/user/my_customize_order', { params });
...
const handleTableChange = (pag, filters, sorter) => {
  ...
  fetchData(activeKey.value,pag.current, pag.pageSize, searchKeyword.value,selectedOrderStatus.value,selectedStatus.value,startDate.value,endDate.value);
};

// 获取数据
const fetchData = async (customize_order_type=activeKey.value,page = 1, pageSize = 10, keyword = searchKeyword.value,order_status=selectedOrderStatus.value,status =selectedStatus.value, start_date = startDate.value,
                         end_date = endDate.value) => {
  loading.value = true;
  try {
    const res = await request.get('/user/my_customize_order', {
      params: {
        page,
        limit: pageSize,
        key: keyword,
...
const handleOrderStatusChange = (value) => {
  ...
  fetchData(activeKey.value,1, pagination.pageSize, searchKeyword.value,value,selectedStatus.value,startDate.value, endDate.value);
};
...
const handleStatusChange = (value) => {
  // value 是 user_id，可能是 undefined
  pagination.current = 1;
  fetchData(activeKey.value,1, pagination.pageSize, searchKeyword.value,selectedOrderStatus.value,value,startDate.value, endDate.value);
};
...
const onDateChange = (dates) => {
  ...
  pagination.current = 1;
  // 搜索时回到第一页
  fetchData(activeKey.value,1, pagination.pageSize, searchKeyword.value, selectedOrderStatus.value,selectedStatus.value, startDate.value,
      endDate.value);
};
...
const handleSearch = () => {
  ...
  pagination.current = 1;
  // 搜索时回到第一页
  fetchData(activeKey.value,1, pagination.pageSize, searchKeyword.value,selectedOrderStatus.value,selectedStatus.value,startDate.value,endDate.value);
};
watch(searchKeyword, (val) => {
  if (val === '') {
    // 可选：清空搜索框时自动搜索（清空结果）
    fetchData(activeKey.value,1, pagination.pageSize, '',selectedOrderStatus.value,selectedStatus.value,startDate.value,endDate.value);
  }
});
watch(activeKey, (val) => {
  // 可选：清空搜索框时自动搜索（清空结果）
