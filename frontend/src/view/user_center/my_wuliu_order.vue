<template>
  <a-card style="padding: 10px" title="头程国际物流明细页">
    <!-- 搜索区域 -->
...
const pagination = reactive({
  ...
  onShowSizeChange: (current, size) => {
    ...
    fetchData(current, size, searchKeyword.value, startDate.value, endDate.value);
  },
});
...
const fetchData = async (
  ...
  loading.value = true;
  try {
    const res = await request.get('/user/my_wuliu', {
      params: { page, limit: pageSize, key: keyword, start_date, end_date },
    });
...
watch(searchKeyword, (val) => {
  if (val === '') {
    fetchData(1, pagination.pageSize, '', startDate.value, endDate.value);
  }
});
...
const fetchAllTransactionData = async () => {
  ...
  while (page <= maxPages) {
    try {
      ...
      const res = await request.get('/user/my_wuliu', { params });
      if (res.data.code !== 200) {
