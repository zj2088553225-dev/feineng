...
<template>
  <a-card style="padding: 10px" title="合伙人Jumia订单明细页">
    <!-- 搜索区域 -->
...
const fetchAllTransactionData = async () => {
  ...
  while (page <= maxPages) {
    try {
      ...
      const res = await request.get('/user/transactions', { params });
...
const pagination = reactive({
  ...
  onShowSizeChange: (current, size) => {
    ...
    fetchData(current, size, searchKeyword.value,selectedPartnerId.value,selectedCountry.value,selectedStatus.value,startDate.value,endDate.value,selectedTransactionType.value);
...
const onDateChange = (dates) => {
  ...
  // 搜索时回到第一页
  fetchData(1, pagination.pageSize, searchKeyword.value, selectedPartnerId.value,selectedCountry.value,selectedStatus.value, startDate.value,
      endDate.value,selectedTransactionType.value);
};
const tableKey = ref(1);
// 获取数据
const fetchData = async (page = 1, pageSize = 10, keyword = searchKeyword.value,partnerId = selectedPartnerId.value,country_code=selectedCountry.value,paid_status =selectedStatus.value, start_date = startDate.value,
                         end_date = endDate.value,transaction_type=selectedTransactionType.value) => {
  tableData.value = []
  loading.value = true;
  try {
    const res = await request.get('/user/transactions', {
      params: {
        page,
        limit: pageSize,
        key: keyword,
...
const handleSearch = () => {
  ...
  pagination.current = 1;
  // 搜索时回到第一页
  fetchData(1, pagination.pageSize, searchKeyword.value,selectedPartnerId.value,selectedCountry.value,selectedStatus.value,startDate.value,endDate.value,selectedTransactionType.value);
};
...
const handleTableChange = (pag, filters, sorter) => {
  ...
  fetchData(pag.current, pag.pageSize, searchKeyword.value,selectedPartnerId.value,selectedCountry.value,selectedStatus.value,startDate.value,endDate.value,selectedTransactionType.value);
};
watch(searchKeyword, (val) => {
  if (val === '') {
    // 可选：清空搜索框时自动搜索（清空结果）
    fetchData(1, pagination.pageSize, '',selectedPartnerId.value,selectedCountry.value,selectedStatus.value,startDate.value,endDate.value,selectedTransactionType.value);
  }
});
...
const handlePartnerChange = (value) => {
  ...
  pagination.current = 1;
  fetchData(1, pagination.pageSize, searchKeyword.value, value,selectedCountry.value,selectedStatus.value,startDate.value, endDate.value,selectedTransactionType.value);
};
...
const handleCountryChange = (value) => {
  // value 是 user_id，可能是 undefined
  pagination.current = 1;
  fetchData(1, pagination.pageSize, searchKeyword.value,selectedPartnerId.value,value,selectedStatus.value,startDate.value, endDate.value,selectedTransactionType.value);
};
...
const handleTransactionTypeChange = (value) => {
  // value 是 user_id，可能是 undefined
  pagination.current = 1;
  fetchData(1, pagination.pageSize, searchKeyword.value,selectedPartnerId.value,selectedCountry.value,selectedStatus.value,startDate.value, endDate.value,value);
};
...
const handleStatusChange = (value) => {
  // value 是 user_id，可能是 undefined
  pagination.current = 1;
  fetchData(1, pagination.pageSize, searchKeyword.value, selectedPartnerId.value,selectedCountry.value,value,startDate.value, endDate.value,selectedTransactionType.value);
};
// 初始化
// 初始化
onMounted(() => {
