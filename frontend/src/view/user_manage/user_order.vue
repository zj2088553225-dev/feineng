<template>
  <a-card style="padding: 10px" title="合伙人Jumia订单页">
    <!-- 搜索区域 -->
...
const columns = [
  ...
  {
    title: '订单金额',
    dataIndex: 'TotalAmountLocalValue',
    key: 'amount',
    width: 120,
...
// 获取数据
const fetchData = async (page = 1, pageSize = 10, keyword = searchKeyword.value, sort = '',partnerId = selectedPartnerId.value,country_name=selectedCountry.value,status =selectedStatus.value,start_date=startDate.value,end_date=endDate.value) => {
  loading.value = true;
  try {
...
const handleSearch = () => {
  ...
  fetchData(1, pagination.pageSize, searchKeyword.value, currentSort.value,selectedPartnerId.value,selectedCountry.value,selectedStatus.value,startDate.value,endDate.value);
};
...
const handleTableChange = (pag, filters, sorter) => {
  ...
  fetchData(pag.current, pag.pageSize, searchKeyword.value, sort,selectedPartnerId.value,selectedCountry.value,selectedStatus.value,startDate.value,endDate.value);
};

// 监听搜索框
watch(searchKeyword, (val) => {
  if (val === '') {
    fetchData(1, pagination.pageSize, '', currentSort.value,selectedPartnerId.value,selectedCountry.value,selectedStatus.value,startDate.value,endDate.value);
  }
});
...
const handlePartnerChange = (value) => {
  ...
  pagination.current = 1;
  fetchData(1, pagination.pageSize, searchKeyword.value, currentSort.value, value,selectedCountry.value,selectedStatus.value,startDate.value,endDate.value);
};
// ✅ 页面加载时获取一次
getbindUser();
...
const handleCountryChange = (value) => {
  // value 是 user_id，可能是 undefined
  pagination.current = 1;
  fetchData(1, pagination.pageSize, searchKeyword.value, currentSort.value, selectedPartnerId.value,value,selectedStatus.value,startDate.value,endDate.value);
};
...
const handleStatusChange = (value) => {
  // value 是 user_id，可能是 undefined
  pagination.current = 1;
  fetchData(1, pagination.pageSize, searchKeyword.value, currentSort.value, selectedPartnerId.value,selectedCountry.value,value,startDate.value,endDate.value);
};
