...
import (
	...
	"log"
	"strings"
	"sync_data/global" // 替换为您的实际项目路径
	"sync_data/models" // 替换为您的实际项目路径
	"time"
)
...
func FillAllCustomizeOrderNG(batchSize int) (FillResult, error) {
	...
	log.Println("[FillAllCustomizeOrderNG] 开始填充 CustomizeOrderNG 表数据...")

	for {
		// 1. 分页查询 CustomizeOrderGH
		var currentBatch []struct {
			NGID        int    `gorm:"column:ng_id"`
			OrderNumber string `gorm:"column:order_number"`
			JumiaSKU    string `gorm:"column:jumia_sku"`
		}
		err := db.Model(&models.CustomizeOrderNG{}).
			Select("ng_id, order_number, jumia_sku").
			Where("ng_id > ?", lastID).
			Order("ng_id ASC").
			Limit(batchSize).
		...
		if err != nil {
			return result, fmt.Errorf("分页查询 CustomizeOrderNG 失败: %w", err)
		}
		...
		log.Printf("[CustomizeOrderNG] 处理批次: %d 条 (KEID %d - %d)",
			len(currentBatch), currentBatch[0].NGID, currentBatch[len(currentBatch)-1].NGID)
		...
		for _, item := range currentBatch {
			...
			if cleanedJumiaSKU != "" {
				...
				jumiaSKUToKEIDMap[cleanedJumiaSKU] = item.NGID
			}
			...
			orderIDMap[cleanedOrderNumber] = item.NGID
		}

		log.Printf("[FillAllCustomizeOrderNG] 有效订单号: %d, 空订单号: %d, 非空 JumiaSKU: %d",
		...
		if len(orderNumbers) > 0 {
			...
			log.Printf("[FillAllCustomizeOrderNG] 匹配到 %d 条 OrderItem 记录", len(orderItemResults))
		}
		...
		if len(jumiaSKUs) > 0 {
			...
			if err != nil {
				log.Printf("[FillAllCustomizeOrderNG] 查询 UserProduct 失败: %v", err)
			} else {
				log.Printf("[FillAllCustomizeOrderNG] 匹配到 %d 条用户记录", len(userResults))
			}
		}
		...
		for _, item := range currentBatch {
			ngID := item.NGID
			update := map[string]interface{}{"ng_id": ngID}
			updated := false
		...
		// 5. 批量更新
		if len(updates) > 0 {
			dbResult := db.Model(&models.CustomizeOrderNG{}).
			...
			if dbResult.Error != nil {
				log.Printf("[FillAllCustomizeOrderNG] 批量更新失败: %v", dbResult.Error)
			} else {
				log.Printf("[FillAllCustomizeOrderNG] 成功更新 %d 条记录", len(updates))
			}
		}
		...
		if len(notFoundOrderNumbers) > 0 {
			log.Printf("[FillAllCustomizeOrderNG] 未找到的订单号 (%d): %v", len(notFoundOrderNumbers), notFoundOrderNumbers)
		}
		if len(notFoundJumiaSKUs) > 0 {
			log.Printf("[FillAllCustomizeOrderNG] 未找到的 JumiaSKU (%d): %v", len(notFoundJumiaSKUs), notFoundJumiaSKUs)
		}

		log.Printf("[FillAllCustomizeOrderNG] 批次完成: 处理=%d, 更新=%d, 空号=%d, 未匹配订单=%d, 未匹配用户=%d",
			len(currentBatch), batchUpdatedTotal, batchEmptyOrderNumber, batchNotFoundOrderItem, batchNotFoundUser)

		lastID = currentBatch[len(currentBatch)-1].NGID
	}

	result.ElapsedTime = time.Since(startTime)

	log.Printf(`[FillAllCustomizeOrderNG] ✅ 填充完成！
		总计处理: %d
		订单号为空: %d
		未找到订单: %d
		更新 Status: %d
